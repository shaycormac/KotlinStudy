package com.assassin.kotlinlib.tool;
...
import com.assassin.kotlinlib.singleton.KotlinDoubleCheckSingleton;
import com.assassin.kotlinlib.singleton.KotlinEnumSingleton;
import com.assassin.kotlinlib.singleton.KotlinHungerSingleton;
import com.assassin.kotlinlib.singleton.KotlinLazySingleton1;
import com.assassin.kotlinlib.singleton.KotlinLazySingleton2;
import com.assassin.kotlinlib.singleton.KotlinStaticInnerSingleton;
			...
			try {
				...
				switch (sType){
					case TYPE_HUNGRY:
						...
						System.out.println("执行线程："+Thread.currentThread().getName()+"--获取的饿汉式的实例:"+ KotlinHungerSingleton.INSTANCE);
						break;
					case TYPE_LAZY_1:
						System.out.println("执行线程："+Thread.currentThread().getName()+"--获取的单线程懒汉的实例:"+ KotlinLazySingleton1.Companion.getSingleton());
						break;
					case TYPE_LAZY_2:
						System.out.println("执行线程："+Thread.currentThread().getName()+"--获取的暴力锁的实例:"+ KotlinLazySingleton2.Companion.getSingleton());
						break;
					case TYPE_DOUBLE_CHECK:
						System.out.println("执行线程："+Thread.currentThread().getName()+"--获取的双重校验的实例:"+ KotlinDoubleCheckSingleton.Companion.getInstance());
						break;
					case TYPE_STATIC_INNER:
						System.out.println("执行线程："+Thread.currentThread().getName()+"--获取的静态内部类的实例："+ KotlinStaticInnerSingleton.Companion.getInstance());
						break;
					case TYPE_ENUM:
						System.out.println("执行线程："+Thread.currentThread().getName()+"--获取的枚举的实例："+ KotlinEnumSingleton.INSTANCE);
						break;
					default:
						break;
				}
